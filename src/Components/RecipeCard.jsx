...
const RecipeCard = (props) => {
  const {id,imageUrl,title,category,description,chefName} = props.recipe
  ...
  return (
     <Link to={`/recipes/details/${id}`} className='w-55 h-80 mr-6 mb-6 bg-[#242424] overflow-hidden rounded hover:scale-105 duration-150'>
        <img  className='w-full h-35 object-cover' src={imageUrl}/>
         ...
         <h3 className='text-center  text-orange-600 -mt-2 text-lg'><small>Category : {category}</small></h3>
         <h3 className='text-center text-yellow-500 -mt-2 text-lg'><small>Chef's Name : {chefName}</small></h3>
